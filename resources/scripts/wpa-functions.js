...
var WPA = {
	...
	timeToPace: function(milliseconds, meters, format, includeFormat) {
        ...
        var paceSeconds = paceMins > 0 ? (Math.round((pace % paceMins) * 60)) : (Math.floor(pace * 60));
	...
	getGenderDescription: function(gender) {
		return WPA.getProperty('gender_' + (gender ? gender : 'B'));
	},
	...
	renderActivityLinkColumn: function (data, type, full) {
		...
		if(data) {
			data = data.replace('https://connect', 'http://connect');
