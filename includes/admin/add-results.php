...
?>
	...
	jQuery(document).ready(function() {
		...
		WPA.Admin.preSubmitResult = function() {
			// check if we have to remove a pending result
			if(WPA.Admin.pendingResultId) {
				WPA.Ajax.deleteResult(WPA.Admin.pendingResultId, function() {
		...
		WPA.Ajax.setup('<?php echo admin_url( 'admin-ajax.php' ); ?>', '<?php echo $nonce; ?>', '<?php echo WPA_PLUGIN_URL; ?>', '<?php echo $current_user->ID; ?>',  function() {
			...
			jQuery("#athleteName").autocomplete({
				...
				select: function( event, ui ) {
					...
					WPA.Ajax.validateEventEntry(WPA.Admin.selectedEventId, function(result) {
						// success
						...
						if(result.pending) {
							WPA.Admin.pendingResultId = result.resultId;
						}
					}, function(result) {
						// fail
