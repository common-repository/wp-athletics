...
if(!class_exists('WP_Athletics_Admin')) {
	...
	class WP_Athletics_Admin extends WPA_Base {
		...
		public function __construct($db) {
			...
			add_action( 'wp_ajax_wpa_admin_all_athletes', array( $this, 'all_athletes' ) );
			add_action( 'wp_ajax_wpa_admin_all_results_for_athlete', array( $this, 'all_results_for_athlete' ) );
			...
			add_action( 'wp_ajax_wpa_update_league', array( $this, 'update_league' ) );
			add_action( 'wp_ajax_wpa_get_leagues', array( $this, 'get_leagues' ) );
			add_action( 'wp_ajax_wpa_delete_league', array( $this, 'delete_league' ) );
			add_action( 'wp_ajax_wpa_modify_league_events', array( $this, 'modify_league_events' ) );
			add_action( 'wp_ajax_wpa_generate_league_page', array( $this, 'generate_league_page' ) );
			add_action( 'wp_ajax_wpa_get_league_events', array( $this, 'get_league_events' ) );
			add_action( 'wp_ajax_wpa_get_league_details', array( $this, 'get_league_details' ) );
		...
		function admin_menu() {
			if( $this->has_permission_to_manage( false ) ) {
				...
				add_submenu_page( 'wp-athletics-settings', 'WP Athletics Leagues', 'Leagues', 'manage_wp_athletics', 'wp-athletics-leagues', array( $this, 'wpa_leagues') );
		...
		/**
		 * [AJAX] Performs request for all athletes
		 */
		function all_athletes() {
		
			// perform the query
			$result = $this->wpa_db->all_athletes( $_POST );
		 ...
		 */
		public function all_results_for_athlete() {
		
			// perform the query
			$results = $this->wpa_db->get_all_results_for_user( $_POST['id'] );
		...
		/**
		 * [AJAX] Gets a list of league events
		 */
		function get_league_events() {
			// perform the query
			$result = $this->wpa_db->get_league_events( $_POST['id'] );
		...
		/**
		 * [AJAX] Creates or updates a league
		 */
		function update_league() {
			// perform the query
			$result = $this->wpa_db->update_league( $_POST );
		...
		/**
		 * [AJAX] Generates a league page
		 */
		function generate_league_page() {
			$success = false;
			$page_id = null;
			...
			if($id && $name) {
				...
				if($page_id) {
					$success = $this->wpa_db->update_league_page( $id, $page_id );
				}
			}
		...
		/**
		 * [AJAX] Deletes a league
		 */
		function delete_league() {
			// perform the query
			$result = $this->wpa_db->delete_league( $_POST['id'] );
		...
		/**
		 * [AJAX] Adds or removes events to a league
		 */
		function modify_league_events() {
			// perform the query
			$result = $this->wpa_db->modify_league_events( $_POST );
		...
		/**
		 * [AJAX] Gets a list of all league
		 */
		function get_leagues() {
			wpa_log('updating league');
			// perform the query
			$result = $this->wpa_db->get_leagues( $_POST );
		...
		/**
		 * [AJAX] Gets details and events for a single league
		 */
		function get_league_details() {
			wpa_log('updating league');
			// perform the query
			$result = $this->wpa_db->get_league_details( $_POST['id'] );
		...
		/**
		 * Generates a page for league management
		 */
		function wpa_leagues() {
			require 'admin/leagues.php';
		}
