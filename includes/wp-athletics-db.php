...
if(!class_exists('WP_Athletics_DB')) {
	...
	class WP_Athletics_DB {
		...
		public function __construct() {
			...
			$this->LEAGUE_TABLE = $wpdb->prefix . "wpa_league";
			$this->LEAGUE_EVENT_TABLE = $wpdb->prefix . "wpa_league_event";
		...
		public function create_db( $force = false ) {
			...
			if( $force || $installed_ver != WPA_DB_VERSION ) {
				...
				CREATE TABLE $this->LEAGUE_TABLE (
				id mediumint(9) NOT NULL AUTO_INCREMENT,
				name tinytext NOT NULL,
				details text,
				rule varchar(20) DEFAULT 'ALL',
				rank_by varchar(20) NOT NULL,
				...
				CREATE TABLE $this->LEAGUE_EVENT_TABLE (
				id mediumint(9) NOT NULL AUTO_INCREMENT,
				league_id mediumint(9) NOT NULL,
				event_id mediumint(9) NOT NULL,
				UNIQUE KEY id (id),
				KEY wpa_leagues_league_idx (league_id),
				KEY wpa_leagues_event_idx (event_id)
				);
		...
		public function search_results( $request ) {
			...
			if( !WPA_DB_DISABLE_SQL_VIEW ) {
				$results = $wpdb->get_results(
					"
					SELECT r.id, user_id, pending, age_grade, u.display_name as athlete_name, time, age_category, gender, garmin_id, position, event_id,
				...
				$results = $wpdb->get_results(
					"
					SELECT r.id, user_id, r.pending, r.age_grade, u.display_name AS athlete_name, r.time, r.age_category, r.gender, r.garmin_id, r.position, r.event_id,
		...
		public function all_athletes() {
			...
			return $wpdb->get_results(
				...
				(SELECT meta_value FROM wp_usermeta um WHERE um.user_id = u.id AND um.meta_key = 'wp-athletics_dob') as dob,
				(SELECT meta_value FROM wp_usermeta um WHERE um.user_id = u.id AND um.meta_key = 'wp-athletics_gender') as gender
		...
		/**
		 *  Function to update all of a users age grades
		 */
		public function update_age_grades( $data ) {
			global $wpdb;
			$count = 0;
		...
		public function get_personal_bests( $request ) {
			...
			if( isset( $rankings_display ) && $rankings_display != '' ) {
				if( $rankings_display == 'best-athlete-result' ) {
					$group_by_and_order = 'GROUP BY user_id';
				}
			}
			...
			JOIN (
			SELECT r.id, r.gender, r.age_category, r.age_grade, r.time AS time, r.garmin_id, r.user_id, ec1.id AS event_cat_id, e.name AS event_name, e.location as event_location,
		...
		/**
		 * Retrieves a list of all existing leagues
		 */
		public function get_leagues( $data ) {
			global $wpdb;
			return $wpdb->get_results( "SELECT id, name FROM $this->LEAGUE_TABLE ORDER BY created_on DESC" );
		}

		/**
		 * Gets league details and events for a single league
		 */
		public function get_league_details( $id ) {
			global $wpdb;
			return array(
				'count' => $wpdb->get_var("SELECT count(id) FROM $this->LEAGUE_EVENT_TABLE WHERE league_id = $id"),
				'league' => $wpdb->get_row("SELECT id, name, details, rank_by, gender, rule, age_category, page_id FROM $this->LEAGUE_TABLE WHERE id = $id")
			);
		}
		...
		/**
		 * Gets a list of events for a given league
		 */
		public function get_league_events( $league_id ) {
			global $wpdb;
			return $wpdb->get_results( "SELECT e.id, ec.name as event_cat, e.name, e.event_cat_id, date_format(e.date,'" . WPA_DATE_FORMAT . "') as date, (CASE WHEN (e.date > DATE(NOW())) THEN 1 ELSE 0 END) AS is_future 
				FROM $this->LEAGUE_EVENT_TABLE le JOIN $this->EVENT_TABLE e on e.id = le.event_id 
				JOIN $this->EVENT_CAT_TABLE ec on ec.id = e.event_cat_id	
				WHERE le.league_id = $league_id ORDER BY e.date DESC" );
		}
		
		/**
		 * Retrieves all possible results for a given league
		 * @param unknown_type $league_id
		 */
		public function get_league_results( $league_id, $league_details = null ) {
			global $wpdb;
			if(!$league_details) $league_details = $this->get_league_details($league_id);
			if($league_details) {
				$league = $league_details['league'];
				$age_cat = $league->age_category;
				$gender = $league->gender;
				$where = "";
				
				if($age_cat) {
					$where .= "and r.age_category = '$age_cat'";		
				}
				...
				$sql = "SELECT r.user_id, u.display_name as athlete_name, r.event_id, e.location, r.time, r.garmin_id, r.position, r.age_category, r.gender, r.age_grade, date_format(e.date,'%d/%m/%y') AS event_date, 
				e.name as event_name, ec.name AS event_cat, ec.time_format, ec.distance_meters, e.sub_type_id FROM $this->RESULT_TABLE r join $this->EVENT_TABLE e ON e.id = r.event_id 
				JOIN $this->EVENT_CAT_TABLE ec ON ec.id = e.event_cat_id JOIN $this->USER_TABLE u ON r.user_id = u.id 
				WHERE r.event_id IN (SELECT event_id FROM $this->LEAGUE_EVENT_TABLE WHERE league_id = $league_id) AND r.pending = 0 $where";
		...
		/**
		 * Adds or removes an event to a league
		 */
		function modify_league_events( $data ) {
			global $wpdb;
			$success = false;
			$error = false;
				
			$action = isset($data['modify']) ? $data['modify'] : null;
			$league_id = isset($data['leagueId']) ? $data['leagueId'] : null;
			$event_id = isset($data['eventId']) ? $data['eventId'] : null;
				
			if($league_id && $event_id) {
				if($action == 'add') {
					$exists = $wpdb->get_var( "SELECT COUNT(id) FROM $this->LEAGUE_EVENT_TABLE WHERE league_id = $league_id AND event_id = $event_id" );
					if($exists) {
						$error = $this->parent->get_property("leagues_error_event_already_added");
					}
					else {
						$success = $wpdb->query( $wpdb->prepare(
							"INSERT INTO $this->LEAGUE_EVENT_TABLE ( league_id, event_id ) VALUES ( %d, %d )",
							$league_id, $event_id
						) );
					}
				}
					...
					$success = $wpdb->query(
						$wpdb->prepare(
							"
							DELETE FROM $this->LEAGUE_EVENT_TABLE
							WHERE league_id = %d AND event_id = %d
							",
							$league_id, $event_id
						)
					);
		...
		/**
		 * Updates the page ID for a league
		 */
		public function update_league_page( $id, $page_id ) {
			global $wpdb;
			return $wpdb->update(
				$this->LEAGUE_TABLE,
		...
		/**
		 * Creates or updates an league with the supplied data
		 */
		public function update_league( $data ) {
			global $wpdb;
			$is_update = (isset( $data['leagueId'] ) && $data['leagueId'] != '0');
			...
			if( !$is_update ) {
				$success = $wpdb->query( $wpdb->prepare(
					"
					INSERT INTO $this->LEAGUE_TABLE
					( name, details, rank_by, gender, age_category, created_on )
					VALUES ( %s, %s, %s, %s, %s, %s )
					",
					$data['leagueName'],
					$data['leagueDetails'],
					$data['leagueRankBy'],
					$data['leagueGender'],
					$data['leagueAgeCategory'],
					$today->format('Y-m-d H:i:s')
				) );
				
				return array(
					'success'=> $success,
					'leagueId' => ($success ? $wpdb->get_var( "SELECT LAST_INSERT_ID()" ) : -1)
				);
			}
			else {
				return array(
					'success' => $wpdb->update(
						...
						array(
							...
							'details' => $data['leagueDetails'],
							'rank_by' => $data['leagueRankBy'],
							'gender' => $data['leagueGender'],
							'age_category' => $data['leagueAgeCategory'],
							'rule' => $data['leagueRule']
						),
						array( 'id' => $data['leagueId'] ),
							array(
							'%s',
							'%s',
							'%s',
		...
		function update_result( $data ) {
			...
			if( !$is_update ) {
				$success = $wpdb->query( $wpdb->prepare(
					...
					( user_id, event_id, time, garmin_id, position, age_category, gender, age_grade )
					VALUES ( %d, %d, %d, %s, %d, %s, %s, %f )
					",
					$data['userId'],
					$data['eventId'],
					$data['time'],
